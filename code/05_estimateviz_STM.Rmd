...
```{r}
dc_preprocessed = read.csv(here("../dph_hearing_decisions/data/dc/intermediate/hearings_preprocessed.csv")) %>%
        mutate(state = "DC",
...
texas_preprocessed = read.csv(here("../dph_hearing_decisions/data/texas/intermediate/hearings_preprocessed.csv")) %>%
        mutate(state = "Texas",
...
print(class(processed_hearings_dc))
...
saveRDS(case_fixk_varymodel_dc, "../output/stm_resultsDC.RDS")



plotModels(case_fixk_varymodel_dc)
selected_model_dc = case_fixk_varymodel_dc$runout[[1]]
summary(selected_model_dc)

## create a table summarizing with the frex metric
...
mycolors <- colorRampPalette(brewer.pal(8, "Dark2"))(nrow(dc_contrast))
...
## create table of topics
print(xtable(dc_contrast %>% dplyr::select(topic, ordered_label)),
      include.rownames = FALSE)

## create representative quotes
findThoughts(selected_model_dc, texts = df_frompy$text_preprocess[df_frompy$state == "DC"],
             topics = c(1, 17), n = 2)
